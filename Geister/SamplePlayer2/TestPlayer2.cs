...
namespace Geister.SamplePlayer2
{
    ...
    /// 
    /// アルゴリズム
    /// 現状態と移動後状態でスコアリングして移動法を決めるサンプルプレイヤー
    /// 初期配置ランダム
    /// goodを取られないように移動
    ...
    class TestPlayer2 : AbstractPlayer
    {
        ...
        public override Move GetMove()
        {
            ...
            else if (myID.Equals(FieldObject.P2))
            {
            ...
            gmlist.OrderBy(j => Guid.NewGuid()).ToList();
            Move m = null;
        ...
        /// </summary>
        /// <returns></returns>
        private Boolean isCatched(Position posi)
        {
            List<Position> enemyGhostList = GetGhostPositionList(enemyID);

            foreach (Position enemyGhost in enemyGhostList)
            {
                if (posi.Row == enemyGhost.Row)
                {
                    if (posi.Col == enemyGhost.Col + 1 || posi.Col == enemyGhost.Col - 1)
                    {
                        return true;
                    }
                }
                if (posi.Col == enemyGhost.Col)
                {
                    if (posi.Row == enemyGhost.Row + 1 || posi.Row == enemyGhost.Row - 1)
                    {
                        return true;
                    }
                }
